    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException {
        ...
        for (Game game : topGames) {
            ...
            try {
                ...
                // 创建一个新的RecommendationException 这样说明错误只来自于这里
            }
            for (Item item : items) {
...
// Map to string
//        favoriteItemIds = favoriteItems.stream().map(new Function<Item, String>() {
//            @Override
//            public String apply(Item item) {
//                return item.getId();
//            }
//        }).collect(Collectors.toSet()); // 把stream 用collection to list
        favoriteGameIds = favoriteDao.getFavoriteGameIds(favoriteItems);


        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) { // loop 3次 video.clip,stream
